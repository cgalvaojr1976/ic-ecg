...
def batimentos(sinal, codigo, frequencia, lead, controle):
    picos = peaks(sinal, frequencia)[0]
    matriz = []
    for posicao in picos:
        ...
        if posicao > 250 and (posicao < (len(sinal) - 400)):
            ...
            lista.append(codigo) # Add na ultima coluna o tipo de batimento
            lista.append(lead) # Add na ultima coluna o tipo de batimento  
        matriz.append(lista)
    if controle == 0:        
        return np.savetxt("record.csv",matriz, delimiter= ',')
    else:
